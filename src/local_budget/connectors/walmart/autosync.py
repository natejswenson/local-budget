...
1. **A failed sync must never fail the report.** The Walmart session expires on
   Walmart's schedule, and re-authenticating needs a human, a browser and a
   one-time code. A month-end report that cannot render because a scraper's
   session went stale is a worse product than one rendered from slightly older
   item data. Every failure is caught and reported as a note.

2. **Only refresh when fresh data could change the answer.** Re-rendering a 2024
   report gains nothing from pulling the last 60 days of orders, and a full
   backfill would turn a two-second render into a long network job nobody asked
   for. Auto-sync applies to the current and previous month only.

3. **Don't re-sync on every render.** Rendering three reports in a row should hit
   Walmart once. A recent successful run is treated as good enough — and the
   bar is higher here than for Amazon: each Walmart order costs its own detail
   request, so a redundant sync is expensive as well as rude.
...
def last_success_at(conn) -> datetime | None:
    row = conn.execute(
        "SELECT completed_at FROM walmart_sync_runs "
...
def maybe_sync(period: str, *, max_age_hours: int = DEFAULT_MAX_AGE_HOURS,
    ...
    """Refresh Walmart data if it would help and is possible.
    ...
    try:
        ...
        if not stored_session_looks_valid():
            return {"status": "no-session",
                    "detail": "no saved Walmart session — run `budget walmart login`"}
        ...
        # Deliberately broad. Anything from an expired session to Walmart being
