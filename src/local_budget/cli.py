...
@click.group()
def main() -> None:
    """Local-first the bank spending agent."""
...
def setup() -> None:
    ...
    click.echo("\nNext:\n"
               "  • budget import <wf-export.qfx>   – load a the bank export\n"
...
def import_cmd(file: Path, detect_duplicates: bool) -> None:
    """Import a the bank OFX/QFX (or CSV) export (rule-based categorization)."""
    db.init_schema()
...
@main.command()
def intake() -> None:
    """Import new the bank exports from your inbox folder and categorize."""
    ...
    if r["files_quarantined"]:
        click.secho(f"  ⚠ {r['files_quarantined']} file(s) skipped — not a recognized the bank "
...
def set_inbox(folder: str | None) -> None:
    """Show or set the folder the app watches for the bank exports."""
    from . import inbox_adapter
    db.init_schema()
    if folder:
        db.set_setting("inbox_dir", str(Path(folder).expanduser()))
    click.echo(f"  inbox folder: {inbox_adapter.inbox_dir()}")
    click.echo("  drop the bank OFX/QFX/CSV exports here, then run `budget intake`")
