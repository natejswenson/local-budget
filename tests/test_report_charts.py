...
_OVERVIEW = {"month": "2026-06", "categories": [
    ...
    _cat("State529", 60000, 50000, over=False, floor=True, pct=120), # floor met (>100%)
...
def test_floor_rows_do_not_set_the_scale_and_clamp_with_a_clip_mark():
    ...
    stubs — every ordinary row became a three-pixel nub."""
    out = charts.spend_vs_budget({"month": "2026-07", "categories": [
        _cat("Savings", 1000000, 250000, floor=True, pct=400),
        _cat("CatA", 120000, 60000, over=True, pct=200),
        _cat("CatB", 50000, 100000, pct=50),
    ]})
    # scale is set by the ceiling rows: max(120000, 50000, 60000, 100000)
    shopping = out.split("CatA")[1].split("sb-row")[0]
    ...
    small = out.split("CatB")[1].split("sb-row")[0]
    g_fill = float(small.split('class="sb-fill" style="width:')[1].split("%")[0])
    assert 40.0 < g_fill < 50.0                      # legible, not a stub
    # the floor row clamps and says so
    inv = out.split("Savings")[1].split("sb-row")[0]
    assert 'style="width:100.0%"' in inv and 'class="clip"' in inv
    assert "$10,000.00 of $2,500.00 · 400%" in inv   # the truth is in the text
...
def test_scale_falls_back_when_every_row_is_a_floor_row():
    ...
    out = charts.spend_vs_budget({"month": "2026-07", "categories": [
        _cat("Savings", 1000000, 250000, floor=True, pct=400),
        _cat("Savings2", 10000, 10000, floor=True, pct=100),
    ]})
    ny529 = out.split("Savings2")[1].split("sb-row")[0]
...
def test_negative_spend_floor_row_keeps_its_sign_in_the_text():
    ...
    out = charts.spend_vs_budget({"month": "2026-07", "categories": [
        _cat("Savings", -500, 300000, over=True, floor=True, pct=0),
        _cat("CatA", 120000, 60000, pct=200),
    ]})
    inv = out.split("Savings")[1].split("sb-row")[0]
