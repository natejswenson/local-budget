...
def test_the_keyword_heuristic_fills_the_gap_when_walmart_publishes_nothing(conn):
    ...
    buckets are the ledger's own category names — not a taxonomy of the
    report's own invention. "Dog food" lands in Pets rather than Groceries
    because Pets leads that table; the keyword "food" would otherwise take it."""
    _order(conn, "O", "2026-07-01", [("P", "Dog food 24lb", 4299, 1, "W", None)])
    _charge(conn, 1, "2026-07-01", -4299)
    _match(conn, "O", 1)
    d = report.gather(conn)
    assert dict(d["by_kind"]) == {"Pets": 4299}
