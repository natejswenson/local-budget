...
def test_remove_custom_category_deletes_from_custom_list(data_dir):
    db.init_schema()
    categories.add_custom_category("Legal")
    assert "Legal" in categories.custom_categories()
    manual.remove_category("Legal", "Fees")
    assert "Legal" not in categories.custom_categories()
    assert "Legal" not in categories.spend_categories()
