...
- `invention_rate(transcript) -> float` — **scoped to CURRENCY tokens only** (`$N` in `final_text`). For each `$` amount: normalize to integer cents and count it as INVENTED unless it is (a) within a rounding tolerance of a tool-result leaf (e.g. "about $500" vs a leaf of $321.65), or (b) a member of a closed set of DERIVED values: sums/deltas of tool-result leaves (net = income − spend, category subtotals). Bare numbers, percentages, counts, and month tokens are NOT currency and are EXCLUDED (they false-positived the old `==0` rule). Returns the invented-fraction as a WARNING/score — see the gate note below. (`fmt_cents`/tolerance constants live in the harness.)
