...
1. **Recurring flags narrow from "any spend-eligible category" to a
   ...
   "Insurance", "Housing", "State529", "Sewer/Water/Trash"}`. Correction on the
   ...
   "recurring software/services"). `State529` and `Sewer/Water/Trash` are the
   ...
   keeps Chewy.com, Verizon (Utilities), STATE 529 ACH (State529), and FUCHS
...
2. **"Where it goes" and "Budget vs. actual" merge into one bullet-style
   ...
   with no budget set at all (e.g. Large Purchases: $2,222.22 spent, no
   ...
   sorting by its actual negative figure (e.g. Sports's -$199.00 row
   ...
   Sports's lone March 2026 transaction was a refund/credit with no
   offsetting charge, netting -$199.00) is treated the same as the exactly-
   $0.00 case below, not
   ...
   **Where the row's numbers are extracted from.**
   ...
   netting to a NEGATIVE Spent value — verified live: Sports's lone
   March 2026 transaction was a refund/credit with no offsetting charge,
   netting -$199.00. A negative value is technically nonzero, but a
   ...
   Sports's March 2026 row — spent -$199.00, budget $555.55, `over`
   ...
   **Shared scale, not per-row scale.** Both the bar length and the tick
   ...
   2026: Housing's budget ($4,444.44) exceeds every category's actual June
   spend (Home Improvement tops out at $3,333.33) — a per-row scale (each
...
- `budget-visualizer/SKILL.md`'s Recipe 3 (flags, renumbered from 4) states
  ...
  `{Subscriptions, Utilities, Insurance, Housing, State529,
...
- Any generic mechanism for the skill to auto-detect a custom category as
  "bill-like" — the `State529` and `Sewer/Water/Trash` allowlist entries are
